...
public class FrogJmp {
    ...
    private boolean XOrDIsGreater(int X, int Y) { return (X >= Y);}

    private boolean thereIsARest(int diff, int D) { return ((diff % D) > 0);}

    public int solution(int X, int Y, int D) {

        if(XOrDIsGreater(X,Y)) return 0;
        if(XOrDIsGreater(D,(X + Y))) return 1;

        int diff = Y - X;
        int result = diff/D;

        if(thereIsARest(diff,D)) result += 1;
