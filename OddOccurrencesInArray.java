...
public class OddOccurrencesInArray {
    ...
    private boolean arrayLengthIsOne(int length) { return (length == 1);}

    private static int getUnpairedNumber(int[] A) {
        int setSize = (A.length/2) + 1;
        Set<Integer> setOfUnpairedNumbers = new HashSet<>(setSize);

        for (int j : A) {
            if (setOfUnpairedNumbers.isEmpty()) setOfUnpairedNumbers.add(j);
            else if (!setOfUnpairedNumbers.add(j)) setOfUnpairedNumbers.remove(j);
        }

        return setOfUnpairedNumbers.iterator().next();
    }

    public int solution(int[] A) {

        if(arrayLengthIsOne(A.length)) return A[0];

        return getUnpairedNumber(A);
    }
