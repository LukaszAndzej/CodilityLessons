...
* moment when all the positions from 1 to X are covered by leaves). You may assume that the speed of the
...
public class FrogRiverOne {
    ...
    private boolean doesntNeedToJump(int[] A) { return (A.length == 1);}

    private boolean frogIsNeverAbleToJump(int result) { return (result == 0);}

    private int getEarliestTime(int[] A, int X) {
        Set<Integer> setOfLeaves = new HashSet<>();

        int earliestTime = 0;
        for(int i = 0; i < A.length; i++) {
            if(setOfLeaves.add(A[i]) && setOfLeaves.size() == X) { earliestTime = i; break;}
        }
    ...
    public int solution(int X, int[] A) {
        ...
        if(doesntNeedToJump(A) && A[0] == X) return 0;
        if(doesntNeedToJump(A)) return -1;

        int result = getEarliestTime(A,X);

        if(frogIsNeverAbleToJump(result)) return -1;
        else return result;
    }
